...
```{r assoc, echo = TRUE}
dat <- subset(phen, !is.na(ab0))
m <- relmatLmer(APTT ~ AGEsc + (1|ID), dat, relmat = list(ID = dkin))
```

```{r assoc_anova, echo = TRUE}
m1 <- update(m, . ~. + ab0)
anova(m, m1)
```

# lme4qtl is <span class="emph">flexible</span> | AB0 affects both mean and variance of APTT <br/> *Do not confuse with model in [@Chen2007]
