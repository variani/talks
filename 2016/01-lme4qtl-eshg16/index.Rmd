...
dat <- subset(phen2, !is.na(ABOf3num))

m <- relmatGlmer(Throm ~ AGEsc + SEXf + ABOf3num + (1|ID), dat, 
  relmat = list(ID = dkin2), family = binomial)
m0 <- relmatGlmer(Throm ~ AGEsc + SEXf + (1|ID), dat, 
  relmat = list(ID = dkin2), family = binomial) 
```
...
m0 <- relmatGlmer(Throm ~ AGEsc + SEXf + ABOf3num + (1|ID), dat, 
  relmat = list(ID = dkin2), family = binomial) 
```
...
## Disease prevalence 5%, coef. (t-test)
...
## Disease prevalence 5%, coef. (anova) 
...
```{r offset_coef_anova_show, echo = TRUE}
anova(m, m0) # offset = log(K/(1 - K)) = -2.944439
```

## Summary on ABO covariate effect
